 ...
 useEffect(()=>{
  ...
  return (
    <div>
        ...
        <h2 className='mt-5 text-[17px] font-semibold'>{watchMode?<span className='flex sm:flex-row flex-col justify-end items-end '>{courceinfo?.chapter[activeChapter]?.name}
        <Button className='text-white sm:text-[17px] text-[10px] font-semibold md:lg:xl:xxl:w-[100px]' onClick={()=>setChapterCompleted(courceinfo?.chapter[activeChapter]?.id)}>Mark Completed</Button>
